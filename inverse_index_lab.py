...
movies = ["See it!", "A gem!", "Ideological claptrap!"]
...
def makeInverseIndex(strlist):
    ...
    for i,item in enumerate(strlist):
        ...
        print(split)
        for word in split:
            # map[word] = map[word] | {i} if word in map else map[word] = {i}
            if word in map:
                map[word] = map[word] | {i}
            else:
                map[word] = {i}
...
def orSearch(inverseIndex, query):
    ...
    results = set()
    for word in query:
        results = results | inverseIndex[word]
    return results
