...
first_five_pows_two = { _**2 for _ in {0,1,2,3,4} }
...
value_list_modified_1 = [a[k] if k in a else 'NOT PRESENT' for a in dlist] # <-- Use the same expression here
k = 'Frodo'
value_list_modified_2 = [a[k] if k in a else 'NOT PRESENT' for a in dlist] # <-- as you do here
...
# listdict2dict = dict(zip(names, [id2salary[x] if x in id2salary else 0 for x in range(len(names))]))
listdict2dict = { x:y for i,x in enumerate(names) for j,y in id2salary.items() if i == j }
